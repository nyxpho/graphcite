...
class CitationBERTGraphSAGE(torch.nn.Module):
  def __init__(self, num_features, num_classes, hidden_channels, graph):
      super(CitationBERTGraphSAGE, self).__init__()
      self.bert = AutoModel.from_pretrained('allenai/scibert_scivocab_uncased')
      self.dropout = Dropout(0.1)
      self.merged = Linear(2 * hidden_channels + 768, hidden_channels)  # set up the other FC layer
      self.graph_layer = GraphSAGE(num_features, hidden_channels, 2)
      self.graph = graph
      self.pred = Linear(hidden_channels, num_classes)

  def forward(self, input_ids, attention_mask, token_type_ids, nodes):
      citation = self.citation(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids).last_hidden_state[:, 0, :]  # batch_size, sequence_length, hidden_size; we take the CLS token embedding
      drop_c = self.dropout(citation)
      titles = self.graph_layer(self.graph.x, self.graph.adj_t)
      # now we can reshape `c` and `f` to 2D and concat them
      combined = torch.cat((drop_c.view(drop_c.size(0), -1), titles[nodes[0]], titles[nodes[1]]), dim=1)
